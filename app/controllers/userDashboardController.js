...
exports.getAllJobs = function(req, res) {
	...
	jobsModel.find({ userId: req.body.userId }).sort({ createdAt: -1 }).skip(req.body.skip).limit(20).lean().exec(function(err, jobResponse) {
		...
		if (jobResponse && jobResponse.length) {
			...
			cityModel.find({_id: { $in: cities }},{city: true}).exec(function(err, cityRes) {
